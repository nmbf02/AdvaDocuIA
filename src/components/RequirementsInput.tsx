...
import { UploadedImage, MetadataHeader } from '../types';
import {
  FileText,
  Lightbulb,
  ClipboardPaste,
...
interface RequirementsInputProps {
  ...
  metadata?: Pick<MetadataHeader, 'cliente' | 'nombreProyecto' | 'moduloAplicacion'>;
}
...
const ClearTextButton: React.FC<{ disabled: boolean; onClear: () => void; label?: string }> = ({
  disabled,
  onClear,
  label = 'Vaciar este cuadro',
}) => (
  <button
    type="button"
    disabled={disabled}
    onClick={onClear}
    title={label}
    className="inline-flex items-center justify-center p-1.5 rounded-md text-slate-400 bg-white hover:text-red-600 hover:bg-red-50 border border-slate-200 hover:border-red-200 transition-colors disabled:opacity-30 disabled:hover:text-slate-400 disabled:hover:bg-white disabled:hover:border-slate-200"
  >
...
5. Emisión del reporte diario firmado en papel.`,
  ...
  return (
    <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-sm border border-slate-200/90 p-3 sm:p-4 flex flex-col min-w-0 max-w-full overflow-x-hidden">
      ...
      <div className="flex flex-col border-b border-slate-100 pb-3 mb-4 gap-2 min-w-0">
        <div className="flex items-start gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="text-sm font-bold text-[#0A3D62] leading-snug break-words">
              Notas del requerimiento
            </h2>
            <p className="text-xs text-slate-500 leading-snug">
              Escríbelas, pégalas o sube un Word, TXT o Markdown para analizarlas.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 min-w-0">
          {/* Toggle View Mode */}
          <div className="bg-slate-100 p-0.5 rounded-lg border border-slate-200 flex text-xs min-w-0">
            <button
              type="button"
              onClick={() => setViewMode('structured')}
              className={`px-2 py-1 rounded-md font-semibold transition-all inline-flex items-center gap-1 ${
                viewMode === 'structured'
          ...
          <button
            ...
            className="inline-flex items-center px-2 py-1 text-xs font-medium text-blue-700 bg-blue-50 hover:bg-blue-100 rounded-md border border-blue-200 transition-colors"
            title="Cargar ejemplo con los 4 apartados e imágenes"
          >
            <ClipboardPaste className="w-3.5 h-3.5 mr-1 shrink-0" />
            <span>Ejemplo</span>
          </button>
        </div>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-1 text-xs overflow-x-auto no-scrollbar pb-1 border-b border-slate-200">
            ...
            {[
              ...
              { id: 'cuestionantes', label: 'Cuestionantes' },
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-2 min-w-0">
              <div className="flex flex-wrap items-start justify-between gap-2 min-w-0">
                <div className="flex items-start gap-2 min-w-0 flex-1">
                  <span className="w-6 h-6 rounded-full bg-[#0A3D62] text-white flex items-center justify-center font-bold text-xs shrink-0">
                    1
                  </span>
                  <Compass className="w-4 h-4 text-[#0A3D62] shrink-0 mt-0.5" />
                  <h3 className="text-xs font-bold text-[#0A3D62] uppercase tracking-wide leading-snug break-words min-w-0">
                    Premisa (Antecedentes y Contexto)
                  </h3>
                </div>
                ...
                <ClearTextButton
                  disabled={!sections.premisa.trim()}
                  onClear={() => updateSectionField('premisa', '')}
                />
                </div>
              </div>
          ...
          {(activeApartadoFilter === 'all' || activeApartadoFilter === 'incidencia') && (
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-2 min-w-0">
              <div className="flex flex-wrap items-start justify-between gap-2 min-w-0">
                <div className="flex items-start gap-2 min-w-0 flex-1">
                  <span className="w-6 h-6 rounded-full bg-slate-600 text-white flex items-center justify-center font-bold text-xs shrink-0">
                    2
                  </span>
                  <AlertTriangle className="w-4 h-4 text-slate-600 shrink-0 mt-0.5" />
                  <h3 className="text-xs font-bold text-[#0A3D62] uppercase tracking-wide leading-snug break-words min-w-0">
                    Incidencia (Problema u Oportunidad)
                  </h3>
                </div>
                ...
                {onImagesChange && (
                  <div>
                    ...
                    <button
                      ...
                      className="inline-flex items-center px-2 py-1 text-[11px] font-semibold text-[#0A3D62] bg-white hover:bg-slate-100 border border-slate-300 rounded shadow-xs transition-colors"
                    >
                      <Upload className="w-3 h-3 mr-1 shrink-0" />
                      <span>Adjuntar</span>
                    </button>
                ...
                <ClearTextButton
                  disabled={!sections.incidencia.trim()}
                  onClear={() => updateSectionField('incidencia', '')}
                />
                </div>
              </div>
              ...
              {images.length > 0 && (
                <div className="flex items-center space-x-1.5 flex-wrap pt-1">
                  <span className="text-[10px] font-bold text-slate-600 uppercase">Insertar Imagen:</span>
                  {images.map((img, idx) => (
                    <button
                      key={img.id}
                      type="button"
                      onClick={() => insertImageTagAtText('incidencia', `[IMAGEN_${idx + 1}]`)}
                      className="px-1.5 py-0.5 text-[10px] font-bold bg-[#0A3D62] text-white rounded hover:bg-[#1E5F8A] transition-colors shadow-xs"
          ...
          {(activeApartadoFilter === 'all' || activeApartadoFilter === 'cuestionantes') && (
            <div className="bg-[#1E5F8A]/8 border border-[#1E5F8A]/25 rounded-xl p-3 space-y-2 min-w-0">
              <div className="flex flex-wrap items-start justify-between gap-2 min-w-0">
                <div className="flex items-start gap-2 min-w-0 flex-1">
                  <span className="w-6 h-6 rounded-full bg-[#1E5F8A] text-white flex items-center justify-center font-bold text-xs shrink-0">
                    3
                  </span>
                  <HelpCircle className="w-4 h-4 text-[#1E5F8A] shrink-0 mt-0.5" />
                  <h3 className="text-xs font-bold text-[#0A3D62] uppercase tracking-wide leading-snug break-words min-w-0">
                    Cuestionantes (Preguntas a Resolver)
                  </h3>
                </div>
                ...
                {onImagesChange && (
                  <div>
                    ...
                    <button
                      ...
                      className="inline-flex items-center px-2 py-1 text-[11px] font-semibold text-[#0A3D62] bg-white hover:bg-blue-50 border border-blue-200 rounded shadow-xs transition-colors"
                    >
                      <Upload className="w-3 h-3 mr-1 shrink-0" />
                      <span>Adjuntar</span>
                    </button>
                ...
                <ClearTextButton
                  disabled={!sections.cuestionantes.trim()}
                  onClear={() => updateSectionField('cuestionantes', '')}
                />
                </div>
              </div>
              ...
              {images.length > 0 && (
                <div className="flex items-center space-x-1.5 flex-wrap pt-1">
                  <span className="text-[10px] font-bold text-[#1E5F8A] uppercase">Insertar Imagen:</span>
                  {images.map((img, idx) => (
                    <button
                      key={img.id}
                      type="button"
                      onClick={() => insertImageTagAtText('cuestionantes', `[IMAGEN_${idx + 1}]`)}
                      className="px-1.5 py-0.5 text-[10px] font-bold bg-[#1E5F8A] text-white rounded hover:bg-[#0A3D62] transition-colors shadow-xs"
          ...
          {(activeApartadoFilter === 'all' || activeApartadoFilter === 'flujo') && (
            <div className="bg-emerald-50/50 border border-emerald-200/80 rounded-xl p-3 space-y-2 min-w-0">
              <div className="flex flex-wrap items-start justify-between gap-2 min-w-0">
                <div className="flex items-start gap-2 min-w-0 flex-1">
                  <span className="h-6 w-6 rounded-full text-slate-900 flex items-center justify-center font-bold text-xs bg-[#2ECC71] shrink-0">
                    4
                  </span>
                  <Workflow className="w-4 h-4 text-emerald-800 shrink-0 mt-0.5" />
                  <h3 className="text-xs font-bold text-emerald-950 uppercase tracking-wide leading-snug break-words min-w-0">
                    Flujo Actual (Proceso Vigente)
                  </h3>
                </div>
                ...
                <ClearTextButton
                  disabled={!sections.flujoActual.trim()}
                  onClear={() => updateSectionField('flujoActual', '')}
                />
                </div>
              </div>
      ...
      {/* Hint Footer */}
      <div className="mt-3 flex items-start gap-2 bg-blue-50/80 border border-blue-200/60 rounded-lg p-2.5 text-xs text-blue-900 min-w-0">
