...
export const SlideDeckEditor: React.FC<SlideDeckEditorProps> = ({
  ...
  return (
    <div className="bg-slate-900 border border-slate-700/80 rounded-2xl shadow-xl overflow-hidden flex flex-col min-w-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 min-h-[560px] divide-y lg:divide-y-0 lg:divide-x divide-slate-800">
        
        {/* LEFT COLUMN: Slide Carousel / List (4 cols on lg) */}
        <div className="lg:col-span-4 xl:col-span-3 p-3 bg-slate-950/60 overflow-y-auto max-h-[620px] space-y-2">
          
          <div className="flex items-center justify-between pb-2 border-b border-slate-800 text-xs text-slate-400">
            <span className="font-bold uppercase tracking-wider text-[10px]">Diapositivas ({deck.slides.length})</span>
            <button
              type="button"
              onClick={() => handleAddSlide('bullets')}
              className="inline-flex items-center gap-1 text-[11px] font-bold text-[#2ECC71] hover:underline cursor-pointer"
            >
          ...
          <div className="space-y-2">
            {deck.slides.map((slide, idx) => (
              <div
                ...
                className={`p-2.5 rounded-xl border transition-all cursor-pointer text-left relative group ${
                  activeSlideIndex === idx
                    ? 'bg-slate-800 border-[#2ECC71] shadow-lg shadow-emerald-500/10'
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className={`w-5 h-5 rounded-md text-[10px] font-black flex items-center justify-center shrink-0 ${
                      activeSlideIndex === idx ? 'bg-[#2ECC71] text-slate-950' : 'bg-slate-800 text-slate-400'
                    }`}>
                      {idx + 1}
                    </span>
                ...
                <div className="mt-1 flex items-center justify-between text-[10px] text-slate-400 pl-7">
                  <span className="truncate">{slide.category || slide.layout}</span>
                  <span className="capitalize text-slate-500">{slide.layout}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="lg:col-span-8 xl:col-span-9 p-4 sm:p-6 bg-slate-950 flex flex-col justify-start">
            ...
            <div className="space-y-4 max-w-3xl w-full mx-auto">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b border-slate-800">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="inline-flex items-center gap-1 px-2.5 py-1 text-xs bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg border border-slate-700"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 text-xs bg-slate-900 border border-slate-700 rounded-lg text-white font-bold"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-300 mb-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2 bg-slate-900 p-3 rounded-xl border border-slate-800">
                    <input
                      ...
                      className="w-full px-2.5 py-1.5 text-xs bg-slate-950 border border-slate-700 rounded text-blue-400 font-bold"
                    />
                    <textarea
                      rows={4}
                      ...
                      className="w-full p-2 text-xs bg-slate-950 border border-slate-700 rounded text-slate-200"
                    />
                  </div>
                  ...
                  <div className="space-y-2 bg-slate-900 p-3 rounded-xl border border-slate-800">
                    <input
                      ...
                      className="w-full px-2.5 py-1.5 text-xs bg-slate-950 border border-slate-700 rounded text-emerald-400 font-bold"
                    />
                    <textarea
                      rows={4}
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-2.5 text-xs bg-slate-900 border border-slate-700 rounded-lg text-slate-200 leading-relaxed"
                />
              </div>
